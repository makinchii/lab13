...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException{
		...
		else if (root != null && root.getNodeWithName(parent) != null)
		{
			parentNode = root.getNodeWithName(parent);
		}
		else {
			throw new TreeException("Something went wrong when trying to find parent node.");
		}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Person 1 does not exist");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("Person 2 does not exist");
